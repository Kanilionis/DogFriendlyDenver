...
$(".btn").on("click", function() { //click on "I Love Dogs" button to generate random dog images.
    ...
    .then(function(response) { 
        ...
        var img = $("<img>").attr("src", response.message[randomnum]).width(500).height(500).append("#picture"); 
          $("#picture").empty() //clears previous picture
          $("#picture").append(img); //appends new picture

        var img2 = $("<img>").attr("src", response.message[randomnum2]).width(500).height(500).append("#picture2");
          $("#picture2").empty()
